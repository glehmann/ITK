/*=========================================================================
  ...
  Module:    $RCSfile: itkImageToComponentTreeFilter.hxx,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkImageToComponentTreeFilter_hxx
#define __itkImageToComponentTreeFilter_hxx
...
// needed for the constant boundary condition
template <typename TPixel, typename TIndex, typename TAttribute>
class NumericTraits< ComponentTreeNode< TPixel, TIndex, TAttribute > * >
{
public:
  typedef ComponentTreeNode< TPixel, TIndex, TAttribute > * Self;
  static Self ZeroValue( Self & )
  {
    return NULL;
  }
  static int GetLength( Self & )
  {
    return 1;
  }
};
...
::GenerateData()
{
  ...
  typedef std::deque< typename InputImageType::OffsetValueType >  IndexListType;
...
/*  typename NodeType::Pointer outNode = NodeType::New();
  ...
  typedef typename std::deque< NodeType * > NodePointerList;
  ...
  while( !pixelMap.empty() )
    ...
    for ( typename IndexListType::iterator idxIt = indexes->begin(); idxIt != indexes->end(); ++idxIt )
      ...
      for ( nnIt = nIt.Begin(),  niIt = iIt.Begin(); nnIt != nIt.End(); nnIt++, niIt++) /*&& niIt != iIt.End()*/
        {
...
//   std::cout << "p: " << n->GetPixel()+0.0 << std::endl;
  ...
  assert( output->NodeCountIndexes( n ) == this->GetOutput()->GetRequestedRegion().GetNumberOfPixels() );
...
//   assert( !node2->GetIndexes().empty() );
  ...
  this->GetOutput()->NodeTakeIndexesFrom( node1, node2 );
