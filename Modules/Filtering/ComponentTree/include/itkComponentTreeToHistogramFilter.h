 ...
 *
 *=========================================================================*/
#ifndef __itkComponentTreeToHistogramFilter_h
#define __itkComponentTreeToHistogramFilter_h
...
namespace itk
{
/** \class ComponentTreeToHistogramFilter
...
class ITK_EXPORT ComponentTreeToHistogramFilter:public ProcessObject
{
public:
  /** Standard typedefs */
  typedef ComponentTreeToHistogramFilter     Self;
  typedef ProcessObject                      Superclass;
  typedef SmartPointer< Self >               Pointer;
  typedef SmartPointer< const Self >         ConstPointer;

  /** Run-time type information (and related methods). */
  itkTypeMacro(ComponentTreeToHistogramFilter, ProcessObject);
  ...
  typedef typename NumericTraits< AttributeType >::ValueType ValueType;
  typedef typename NumericTraits< ValueType >::RealType      ValueRealType;

  typedef TImage                                         ImageType;
  typedef typename ImageType::PixelType                  PixelType;

  typedef typename Statistics::Histogram< ValueRealType > HistogramType;
...
public:
   ...
   */
  itkSetDecoratedInputMacro(HistogramSize, HistogramSizeType);
...
//
    ...
    * histogram bins. */
   itkSetDecoratedInputMacro(HistogramBinMinimum, HistogramMeasurementVectorType);
   itkSetDecoratedInputMacro(HistogramBinMaximum, HistogramMeasurementVectorType);
  ...
  itkSetDecoratedInputMacro(AutoMinimumMaximum, bool);
...
protected:
  ...
  virtual ~ComponentTreeToHistogramFilter() {}
  void PrintSelf(std::ostream & os, Indent indent) const;

  /** Triggers the Computation of the histogram */
  void GenerateData(void);
  void ComputeRange( const NodeType * node );
  void ComputeHistogram( const NodeType * node );

  /** Method that construct the outputs */
  DataObject::Pointer  MakeOutput(unsigned int);

private:
  ComponentTreeToHistogramFilter(const Self &); //purposely not implemented
  void operator=(const Self &);         //purposely not implemented

  AttributeType m_Minimum;
  AttributeType m_Maximum;

};
} // end of namespace itk

#ifndef ITK_MANUAL_INSTANTIATION
#include "itkComponentTreeToHistogramFilter.hxx"
#endif
